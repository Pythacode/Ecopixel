...
Il s'agit d'une classe qui contient les variables globales du jeu.
...
- `data` : Les données de la sauvegarde JSON, un dictionaire vide si elle n'existe pas
- `scren` : Écran principal du jeu
- `current_view` : Vue actuellement affiché à l'écran
- `WHITE` et `BLACK` : Couleurs RGB
- `running` : Indique si le jeu tourne
- `main_font_name` : Nom de la police de base du jeu
- `scroll_x` et `scroll_y` : Décalage en x et y du à la molette de la souris
- `touch_pressed` : Un dictionaire avec toute les touche actuellement pressées. Pour savoir si la touche est pressée : `self.touch_pressed.get(<TOUCHE PYGAME>, False)`
- `logo` : Image du logo du jeu
- `back` : Image de la flèche retour du jeu
- `player` : Le joueur
- `key_move_right`, `key_move_left`, `key_action` : Touche enregistrée ou par défaut.
...
Il s'agit d'une classe qui ajoute une zone de texte
...
- `border_radius`: Dureté des coins de la bordure
